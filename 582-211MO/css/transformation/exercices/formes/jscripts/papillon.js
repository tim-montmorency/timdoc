var popup = new AppendPopup({
    ...
    text: "Félicitation, vous maîtrisez transform-origin",
    color: "#03dda2",
    background: "https://media3.giphy.com/media/26DN9QdExGXeOtt9C/giphy.gif?cid=790b7611d5ef54a79be5892275116c0e86d716edf34d7d5b&rid=giphy.gif",
...
body {
    ...
    background-image: url(https://tim-montmorency.com/timdoc/582-215MO/css/transformation/exercices/formes/images/papillon.png);
}
.slide {
    width: 200px;
    height: 200px;
...
var validate = debounce(function () {
    var s1 = validateSlide(red, '-100', '-100', 0, '1.0', '1.0', 100, 100);
    if(s1) {
        var s2 = validateSlide(blue, '-100', '-100', 45, '1.4', '1.4', 0, 0);
        if(s2) {
            var s3 = validateSlide(green, '-100', '-100', 45, '1.4', '1.4', 200, 200);
            if(s3) {
...
var validateSlide = function(el, x, y, angle, scaleX, scaleY, tx, ty) {
    var styles = window.getComputedStyle(el, null);
    var matrix = getMatrix(styles);

    if (!validateTransformOrigin(styles, tx, ty)) return false;
...
var validateTransformOrigin = function(styles, x, y) {
    if (styles.transformOrigin !== x + 'px ' + y + 'px') return false;
    return true;
}
