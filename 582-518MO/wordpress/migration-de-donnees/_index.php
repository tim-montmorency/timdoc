...
<p>Pour avoir accès à l'outil d'exportation de WordPress, il faut aller dans l'onglet <code>Outils</code> et sélectionner l'option <code>Exporter</code>. Une liste de boutons radio permet alors de choisir si l'ensemble du site ou seulement certaines parties doivent être exportés. Cliquer sur le bouton <em>Télécharger le fichier d'export</em> permet alors d'obtenir un fichier XML contenant les données du site.</p>
...
<p>Pour avoir accès à l'outil d'importation de WordPress, il faut aller dans l'onglet <code>Outils</code> et sélectionner l'option <code>Importer</code>. Différentes options permettent alors de choisir la provenance des données à importer <em>(Blogger, Tumblr, etc.)</em>. Si la migration provient d'une autre instance de WordPress, il faut alors choisir l'option <em>WordPress</em>. Dès lors, un champ permettant de charger un fichier XML devrait apparaitre. En important le fichier XML du site désiré, les données de celui-ci devraient alors être recopiées vers le nouveau site.</p>
