...
<p>Les fonctionnalités <code>whiteness</code> et <code>blackness</code> permettent d'altérer une couleur en lui ajoutant du blanc ⚪️ ou du noir ⚫️. Le résultat correspond au résultat obtenu si un galon de peinture verte était mélangé à moitié <em>(50%) </em>avec un galon de peinture blanche ou à moitié <em>(50%)</em> avec un galon de peinture noire.</p>
...
<p>La fonctionnalité <code>saturation</code> permet d'altérer l'intensité d'une couleur. Par exemple, en donnant une saturation de -50%, une couleur devient automatiquement 50% moins vibrante. La fonctionnalité <code>color.grayscale</code> quant à elle est l'équivalent d'une couleur ayant perdu toute son intensité, pour ainsi dire l'équivalent de lorsque cette couleur est imprimée en noir et blanc 🖨. </p><p>Les unités doivent être exprimées en pourcentage<em> (%)</em> pouvant-être positif ou négatif.</p>
...
<p class="codepen" data-height="370" data-theme-id="44431" data-default-tab="css,result" data-slug-hash="rNozemY" data-user="tim-momo" style="height: 370px; box-sizing: border-box; display: flex; align-items: center; justify-content: center; border: 2px solid; margin: 1em 0; padding: 1em;">
  ...
  on <a href="https://codepen.io">CodePen</a>.</span>
    ...
    <p><code>color.adjust($color, $saturation: -100%)</code> = <code>color.grayscale($color)</code>,<br>
...
<p>La fonctionnalité <code>lightness</code> permet d'altérer la luminosité d'une couleur. Il est donc possible de la rendre plus lumineuse ou plus sombre via celle-ci.</p><p>Les unités doivent être exprimées en pourcentage<em> (%)</em> pouvant-être positif ou négatif.</p>
...
<p>La fonctionnalité <code>color.complement</code> permet d'obtenir la couleur complémentaire sur le cercle chromatique à la couleur passée en argument. Tandis que la fonctionnalité <code>color.invert</code> soustrait les valeurs de rouge, vert et bleu à 255 afin d'obtenir la couleur opposée exacte.</p>
...
<p>Les fonctionnalités <code>red</code>, <code>green</code> et <code>blue</code> permettent d'augmenter ou de réduire le taux de rouge 🔴, vert 🟢 ou bleu 🔵 dans une couleur donnée. </p>
...
<p>La fonctionnalité <code>alpha</code> permet de faire fluctuer le taux de transparence d'une couleur. Pour ce faire, il faut passer un nombre entre -1 et 1 s'ajoutant à la valeur courante d'alpha. </p><p>Par exemple, si une couleur est actuellement opaque <em>(alpha de 1)</em>, il faudra lui donner la valeur <code>-0.5</code> pour diminuer son opacité de 50%.</p>
...
<p>Voici coup sur coup la même couleur verte 🟢 <em>(</em><code>$color: #00c774;</code><em>)</em> altérée via des fonctionnalités du module de couleurs Sass:</p>
