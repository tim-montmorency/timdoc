...
<checklist>
Ajoutez la police Roboto</a>: Sélectionnez les niveaux de graisse <em>Thin</em>, <em>Light</em>, <em>Regular</em> et <em>Bold</em>. Appliquez la police sur l'ensemble de votre page et appliquez un graisse par défaut.
...
Sur un écran plus petit que le breakpoint md, on devra voir le bouton avec icône du "Burger menu" aligné tout à droite.
...
Ajoutez une balise <code>main</code> SOUS la balise <code>header</code>. Dans <code>main</code>, ajoutez 3 <code>section</code> une en dessous de l'autre, avec les <code>id</code> suivants: hero, presentation, actalite (sans accent).
...
Styliser la section <code>#hero</code> en css et lui attribuant la couleur de la variable <code>bs-light</code> en arrière-plan.
...
Activez le carrousel en JavaScript avec les configurations suivantes:<br>- Transition par fondu enchaîné<br>- Dois jouer automatiquement et rester 2 secondes à la fois<br>- Doit tourner à l'infini <br>- Doit contenir une navigation précédent et suivant<br>- Doit contenir une pagination dans le bas, comme on le vois dans l'APERÇU.
Stylisez, en CSS, les flèches et navigation et les éléments de paginations pour qu'ils soient de la même couleur que dans l'APERÇU.
...
Par défaut, chaque colonne s'affiche sur toute la largeur de la page.
...
<checklist>
Dans la section<code>#presentation</code> que vous avez préalablement créee, ajoutez les balises et classes nécessaires pour une grille de mise en page Bootstrap: une rangée d'une seule colonne qui inclura le titre et une autre rangée qui contiendra 3 colonnes et une dernière rangée d'une seule colonne qui contiendra le bouton "Toute l'actualité>>".
Dans la rangée 2: Par défaut, chaque colonne s'affiche sur toute la largeur de la page.
...
Stylisez en CSS les listes ul <code>.categories</code> pour qu'elles s'affichent tel que présentées dans l'APERÇU.
Ajoutez des classes utilitaires Bootstrap pour créer l'espacement nécessaire entre chaque élément et section.
...
<checklist>
Sous la balise <code>main</code> ajoutez les balise sémantique pour le pied de page.
Ajoutez les balises et classes nécessaires pour une grille de mise en page Bootstrap de 4 colonnes.
Déposez simplement le contenu textuel qui est prévu pour chaque colonne. Nul besoin de styliser le texte, on arrête là.
Ajoutez des classes utilitaires Bootstrap pour créer l'espacement nécessaire entre chaque élément et section.
</checklist>
