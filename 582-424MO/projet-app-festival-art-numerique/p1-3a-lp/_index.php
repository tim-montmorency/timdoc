...
Dans la 2e rangée, créez 6 colonnes égales. Par défaut, les colonnes s’affichent sur toute la largeur de la rangée. À partir du breakpoint <em>md</em>, on verra 2 colonnes par rangée, et à partir du breakpoint <em>lg</em>, on verra 3 colonnes par rangée (excepté si vous choisissez d'avoir des cartes horizontales, on gardera alors 2 colonnes par rangée pour le breakpoint <em>lg</em> (je fais référence au style de carte du 2e aperçu ci-haut). 
...
Chaque carte Bootstrap devra accueillir une icône (icone Bootstrap). Utiliser la méthode <em>Icon Font</em> (voir documentation liée au point précédent). Vous n'êtes pas contraints à utiliser les même icônes que les aperçus ci-haut. Allez-y avec votre intuition. 
...
<p>Aperçu du résultat selon 3 grandeur d'écran: par défautl, ensuite à partir du breakpoint sm et ensuite à partir du beakpoint lg 👇</p>
...
<checklist>
Sous la section <em>section-features</em>, créez une 3e section, identifiez-la <em>section-testimonials</em>. Ce sera la section des témoignages de gens qui sont contents de l’appli.  
...
La carte contient une photographie de la personne qui témoigne. Vous pouvez utiliser ce <a href=" https://unsplash.com" target="_blank" onclick="event.stopPropagation()">site pour aller chercher des portrait photographique d'hommes et de femmes.</a> Choisissez le format "petit" de l'image. Prenez le temps de télécharger les images et les classer dans vos <em>assets</em>. Appliquez à l’image une hauteur à votre guise autour de height: 20vh; afin que sa dimension soit adaptable avec la dimension de l’écran (vh = view height). Positionnez l’image dans son cadre avec la propriété <code>object-fit</code> et <code>object-position</code>. 
...
Assurez-vous aussi d’alterner la couleur de fond de chacune de vos sections pour qu’on voit bien la séparation de chacune. 
</checklist>
