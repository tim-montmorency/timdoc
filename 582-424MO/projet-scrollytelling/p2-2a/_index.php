 <?php
   ...
   /**
    * @type     article
    * @title    P2.2a Animation chapitre x
    * @icon     images/icon.svg
    * @abstract Animation simple de la composition du chapitre de votre choix
   * @index    20
    */
   ?>
...
La balise <code>header</code> doit contenir une image d’arrière-plan de votre choix. (Une image qui représente bien votre histoire. Ne pas utiliser la même image d'arrière plan que le chapitre 1).</checklist>
 ...
 <p>Vous pouvez accompagner aussi cette icône par un
     ...
     Animez cette icône en boucle afin d’attirer l’attention sur celle-ci. Vous devez utiliser GSAP pour réaliser cette animation. 
...
<grostitre>Page distince pour tester l'animaiton de "Sprite Sheet"</grostitre>


 <p>Créez une page HTML distincte pour tester vos sprite
     sheet. Commencez par une seule <em>Sprite Sheet</em>.</p>



 <h3>Requis HTML</h3>

 <p>Ajoutez les balises de base du HTML.</p>

 <p>Liez votre CSS et votre fichier script.js à
     cette page.</p>
 ...
 <p>Appliquez une hauteur considérable au <code>&lt;body&gt;</code> ou autre
     balise dans votre <code>&lt;body&gt;</code> afin que votre page
     soit <em>scrollable</em> et que vous puissiez tester le défilement de la page et l'animation qui s'active.</p>

 <p>Dans votre fichier CSS,
     créez une animation avec votre <em>Sprite Sheet</em> tel que
     vous l’avez appris. </p>

 <p>Lorsque vous êtes satisfait de l’animation,
     ajoutez une <code><a
href="https://developer.mozilla.org/en-US/docs/Web/CSS/animation-play-state">animation-play-state</a></code>
     afin que l’animation ne s’active pas par défaut.</p>

 <h3>Requis JavaScript</h3>

 <p>Ajoutez un écouteur d’événement <code>scroll</code> sur la
     fenêtre (<code>window</code>). </p>

 <h4>Sur cet événement</h4>

 <p>- ajoutez à la balise <code>&lt;body&gt;</code> la classe <code>.is-scrolling</code></p>

 <p>- créez une variable et storez-y un minuteur ayant un délai de 100ms</p>

 <p>- lorsque ce délai est atteint,
     on enlève la classe <code>.is-scrolling</code> de la balise <code>&lt;body&gt;</code></p>

 <p>- avant de déclarer ce minuteur, vous annuler ce minuteur afin de le réinitialiser tant que
     l’utilisateur défile la page.</p>

 <h3>De retour dans le CSS</h3>

 <p>De retour dans le CSS, vous devez changer l’état
     l’animation de la <em>Sprite Sheet</em> à la valeur «&nbsp;running&nbsp;» seulement lors
     que la balise <code>&lt;body&gt;</code> contient la classe <code>.is-scrolling</code>.</p>
     <p>Assurez-vous que votre élément animé via la technique <em>Sprite Sheet</em> soit <em>responsive</em>, donc adaptable à la largeur de l'écran</p>


 <p>Lorsque tout fonctionne comme vous le souhaitez, faites le
     même exercice pour votre 2<sup>e</sup> sprite sheet (soit dans la même page
     HTML ou dans 2<sup>e</sup> fichier séparé, à votre guise).</p>
